...
/**
 * check_sub_tree_Left - checking if all nodes are smaller than
 * the specified root
 * @node: node for verifying condition in the tree
 * @max: value for comparison
...
/**
 * check_sub_tree_Right - checking if all the nodes are bigger than the
 * specified root
 * @node: node for verifying condition in the tree
 * @min: value for comparison
...
/**
 * binary_tree_is_bst - if a tree is a bst or not
 * the process here is to verify that the left node is smaller than the root
